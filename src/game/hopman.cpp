...
/**
 Set up the game
 */
void Hopman::init(bool server_mode, std::string host) {
...
int Hopman::play() {
    ...
    while (game_state != GameState::EXITING) {
        ...
        Graphics::instance().focusScreenOffsets(players[0]->getRect().getCollider());
        background.updateLayerOffsets(players[0]->getRect().xPos(), players[0]->getRect().yPos());
...
void Hopman::registerInputCallbacks() {
    ...
    // player movement
    Input::instance().registerCallback(Action::MOVE_LEFT, std::bind(&Being::moveLeft, players[0]));
    Input::instance().registerCallback(Action::STOP_LEFT, std::bind(&Being::stopLeft, players[0]));
    Input::instance().registerCallback(Action::MOVE_RIGHT, std::bind(&Being::moveRight, players[0]));
    Input::instance().registerCallback(Action::STOP_RIGHT, std::bind(&Being::stopRight, players[0]));
    Input::instance().registerCallback(Action::JUMP, std::bind(&Being::jump, players[0]));
}
...
void Hopman::createBackground() {
    ...
    background.init(players[0]->getRect().xPos(), players[0]->getRect().yPos(), lower_bound - 200);
...
bool Hopman::isPlayer(Drawable *obj) {
    return std::find(players.begin(), players.end(), obj) != players.end();
}
...
void HopmanClient::handleDeath() {
    ...
    if (players[0]->dead() || players[0]->needsRemoval()) {
        tryRespawn();
    }
}
